#!/usr/bin/env python3
"""R5 (AWQ leg) — quantize the merged extraction model with AWQ (4-bit), then run
the extraction eval on it. Compares to GGUF Q4 (99.5%, R1) and bnb-nf4 (98.8%, R7)."""
import json, re, torch
from transformers import AutoTokenizer
from awq import AutoAWQForCausalLM

SRC = "models/hf/Qwen2.5-0.5B-extract-merged"
OUT = "models/hf/extract-awq"
...
print("AWQ saved, loading for eval...", flush=True)
...
for r, p in zip(rows, preds):
    ...
    ex += ok
print(f"AWQ 4-bit extraction EXACT: {ex/len(rows):.1%}")
print("=== R5 AWQ done ===")
