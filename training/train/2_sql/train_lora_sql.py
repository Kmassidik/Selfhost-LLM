#!/usr/bin/env python3
"""Task 2 — LoRA fine-tune Qwen2.5-0.5B for text -> SQL on the shop schema."""
...
def main():
    ...
    ap.add_argument("--data", default="train/2_sql/data")
    ap.add_argument("--schema", default="train/2_sql/schema.sql")
    ap.add_argument("--out", default="train/2_sql/adapter")
    ...
    def to_pc(r):
        return {"prompt": [{"role": "system", "content": SYS},
                ...
                "completion": [{"role": "assistant", "content": r["sql"]}]}
    ...
    cfg = SFTConfig(output_dir=args.out, num_train_epochs=args.epochs,
                    ...
                    bf16=True, max_length=512, report_to=[], completion_only_loss=True,
                    disable_tqdm=True)
